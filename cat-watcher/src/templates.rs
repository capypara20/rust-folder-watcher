pub const GLOBAL_TOML: &str = r#"# ─── TOML の書き方 ───────────────────────────────────────────────────
# パスを含む値は必ずシングルクオートのリテラル文字列 '...' で書いてください。
# ダブルクオート "..." はバックスラッシュをエスケープとして解釈するため、
#   "C:\temp\new"  →  \t がタブ、\n が改行に化けます（エラーにならないので厄介）
#   "C:\watch"     →  invalid escape sequence でパースエラー
# リテラル文字列なら \ はただの文字なので、この事故が起きません。
# スラッシュ区切り（C:/watch）でも動作します。
...
pub const RULES_TOML: &str = r#"# ─── TOML の書き方 ───────────────────────────────────────────────────
# パスを含む値は必ずシングルクオートのリテラル文字列 '...' で書いてください。
# ダブルクオート "..." はバックスラッシュをエスケープとして解釈するため、
#   "C:\temp\new"  →  \t がタブ、\n が改行に化けます（エラーにならないので厄介）
#   "C:\watch"     →  invalid escape sequence でパースエラー
