...
- [プレースホルダー](#プレースホルダー)
  - [ログのファイル名で使えるプレースホルダー](#ログのファイル名で使えるプレースホルダー)
- [検知の動作仕様](#検知の動作仕様)
...
> ログに出るパスは、どちらで書いても OS の区切り文字に揃えて表示されます（[ログのパス表記](#ログのパス表記)）。
...
アクションの **`destination` / `command` / `args`** で使えます。実行するたびに、その検知の内容で置き換えます。

`program` と `working_dir` では**使えません**（置き換えずにそのまま使うため、起動時の検査でエラーにします）。

例として、`C:/data/sub/report.csv` を検知したとき（監視ルートは `C:/data`）、2024-03-02 10:30:20 に実行した場合:

| プレースホルダー | 内容 | 例 |
|----------------|------|----|
| `{FullName}`      | ファイルのフルパス | `C:/data/sub/report.csv` |
| `{DirectoryName}` | 親フォルダのフルパス | `C:/data/sub` |
| `{Name}`          | ファイル名（拡張子あり） | `report.csv` |
| `{BaseName}`      | ファイル名（拡張子なし） | `report` |
| `{Extension}`     | 拡張子（ドットなし）。拡張子が無ければ空 | `csv` |
| `{RelativePath}`  | 監視ルートからの相対パス | `sub/report.csv` |
| `{WatchPath}`     | 監視ルートのパス | `C:/data` |
| `{Destination}`   | 直前の copy / move の出力先（チェーン用）。まだ無ければ空 | コピー後のフルパス |
| `{Date}`          | 日付（年 4 桁・月 2 桁・日 2 桁） | `20240302` |
| `{Time}`          | 時刻（時 2 桁・分 2 桁・秒 2 桁、24 時間表記） | `103020` |
| `{DateTime}`      | 日付と時刻を `_` でつないだもの | `20240302_103020` |

**日時はいつの時点か**:

- 検知が確定して、そのルールの**アクションを実行し始めた時点**の、PC のローカル時刻です
- 1 回の検知の中では、**すべてのアクションで同じ値**になります（1 つ目の copy と 2 つ目の command で日付がずれることはありません）
- ファイルが作られた時刻や、ファイルの更新日時ではありません

- パス系のプレースホルダーは **Windows でも `/` 区切りに正規化** されます（`C:\data\report.csv` → `C:/data/report.csv`）。コマンドに渡す際に `\` 区切りが必要な場合は注意してください（**ログの表記とは別**です。ログは OS の区切り文字に揃えて出ます）
- `{` `}` をそのまま出力したい場合は `{{` `}}` とエスケープします
- 名前は大文字小文字を区別します（`{date}` はエラー）
- 知らない名前は、起動時の[検査](#検査する内容)でエラーになります

### ログのファイル名で使えるプレースホルダー

`system_log.file_name` と、ルールごとの `log.detect.file_name` / `log.action.file_name` では、
**`{Date}` と `{DateTime}` だけ**が使えます（それ以外はエラー）。

| プレースホルダー | 置き換わる値 | 例 |
|---|---|---|
| `{Date}` | ファイル名を決めた日の日付 | `system_{Date}.log` → `system_20240302.log` |
| `{DateTime}` | ファイル名を決めた日時 | `system_{DateTime}.log` → `system_20240302_103020.log` |

**ファイル名を決めるタイミングは `rotation` で変わります。**

| `rotation` | ファイル名を決めるとき | 結果 |
|---|---|---|
| `never` | **起動したときの 1 回だけ** | 起動してから止めるまで、ずっと同じファイルに書きます。日付が変わっても新しいファイルは作りません |
| `daily` | 起動したときと、**日付が変わったとき** | 1 日 1 ファイルになります。`{DateTime}` なら「起動した時刻」または「日付が変わった後に最初に書き出した時刻」が入ります |

例: 2024-03-02 10:30:20 に起動し、翌日まで動かし続けた場合

| `file_name` | `never` | `daily` |
|---|---|---|
| `system_{Date}.log` | `system_20240302.log` だけ | `system_20240302.log` → 翌日から `system_20240303.log` |
| `system_{DateTime}.log` | `system_20240302_103020.log` だけ | `system_20240302_103020.log` → 翌日から `system_20240303_000005.log` など |
| `system.log`（プレースホルダー無し） | `system.log` だけ | `system.log` だけ（アクションログの `#N` は日付が変わると 1 に戻る） |

- 起動し直すと、その時点でファイル名を決め直します
- アクション側のプレースホルダーと違い、パスの区切り文字の正規化などはありません

> v2.3.0 までは、ファイル名を**ログを書き出すたびに**決めていました。そのため `rotation = "never"` でも日付が変わると別のファイルになり、
> `{DateTime}` では書き出すたびに別のファイルができていました。
...
**ログに出るパスは、その OS が標準とする区切り文字に揃えて表示されます**（Windows なら `\`、Linux なら `/`）。

設定に `/` 区切りで書いた場合、検知したパスは「設定に書いた監視ルート」＋「OS が繋いだ相対部分」になるため、揃えないと区切り文字が混ざります。

```
揃える前:  C:/data/incoming\sub\report.csv     ← 設定由来の / と OS 由来の \ が混在
揃えた後:  C:\data\incoming\sub\report.csv
```

- **表示を揃えるだけで、実際のファイル操作に使うパスは変わりません**
- **プレースホルダーは対象外**です。`{FullName}` などは従来どおり `/` 区切りに正規化されます（コマンドへ渡す値なので、表示とは別の都合があるため）
- Linux では何も置き換えません。**バックスラッシュは Linux では正当なファイル名の文字**なので、区切りとみなして置き換えるとログ上のファイル名が実物と食い違ってしまいます
...
設定に 1 件の問題があります: C:\catwatcher\global.toml
  [1] system_log.dir
      フォルダ 'C:\logs\app' が存在しません
      対処: 先にフォルダを作成するか、存在するフォルダを指定してください
設定に 2 件の問題があります: C:\catwatcher\rules.toml
  [1] rules "csv-backup" > watch.path
      フォルダ 'C:\data\incoming' が存在しません
      対処: 監視するフォルダのパスが正しいか確認してください
  [2] rules "log-processor" > actions[1] > shell
      type = "command" では必須です（コマンドを実行するシェル）
```
...
| 行 | 内容 |
|---|---|
| 1 行目 | **どこの設定か**。`rules "ルール名" > actions[番号] > キー` の形。番号は設定ファイルに書いた順に **1 から** 数えます。`name` が空のルールは `rules[2]` のように何番目かで指します |
| 2 行目 | **何が問題か**。対象の値は `'...'` で囲みます |
| `対処:` | **どうすればよいか**（分かる場合だけ） |

`--validate` を付けずに起動した場合も、監視を始める前に同じ検査をします。問題があれば起動せず、終了コード `10` で終わります。

### 検査する内容

検査は**起動時に 1 回だけ**行います。「設定ファイルの書き方」→「値の中身」の順に見て、書き方の段階で誤りがあれば中身の検査までは進みません。
...
| 項目 | 検査 |
|---|---|
| `system_log.dir` | 空でないこと。**フォルダが既に存在する**こと（自動では作りません） |
| `system_log.file_name` | 空でないこと。プレースホルダーは `{Date}` と `{DateTime}` だけ（[ログのファイル名](#ログのファイル名で使えるプレースホルダー)） |
| `dashboard.bind` | `enabled = true` のときだけ、`127.0.0.1:8080` のような「IP アドレス:ポート番号」として読めること |
| `detect.poll_interval_ms` | 1 以上であること |

#### 3. rules.toml の中身（ルールごと）

| 項目 | 検査 |
|---|---|
| `[[rules]]` | 1 つ以上あること |
| `name` | 空でないこと |
| `watch.path` | **フォルダが存在する**こと |
| `watch.events` | 1 つ以上指定されていること |
| `watch.patterns` / `watch.regex` | **どちらか一方だけ**が指定されていること（両方・どちらも無し はエラー） |
| `exclude_patterns` と `exclude_regex`<br>`dir_patterns` と `dir_regex`<br>`exclude_dir_patterns` と `exclude_dir_regex` | それぞれ同時に指定していないこと |
| glob のパターン | glob として正しいこと（`[` の閉じ忘れなど） |
| 正規表現 | 正規表現として正しいこと |
| `log.detect` / `log.action` | `enabled = true` のとき、`dir` と `file_name` を `system_log` と同じ基準で検査 |
| `actions` | 1 つ以上あること |

#### 4. アクションの中身

**必須の項目**は `type` によって変わります。

| `type` | 必須の項目 |
|---|---|
| `copy` / `move` | `destination` / `overwrite` / `preserve_structure` / `verify_integrity` |
| `command` | `shell` / `command` / `working_dir`（変更しないなら `""`） |
| `execute` | `program` / `args`（引数が無いなら `[]`） / `working_dir`（変更しないなら `""`） |

| 項目 | 検査 |
|---|---|
| `wait` / `timeout_ms` | `command` / `execute` 以外に書いていないこと（`copy` / `move` では効かないため） |
| `destination` | `auto_create = true`（既定）: 絶対パスなら、**親をたどってどこかに存在するフォルダがある**こと（ドライブや共有名の書き間違いを検出）<br>`auto_create = false`: プレースホルダーより前の固定部分が、**フォルダとして存在する**こと<br>`{WatchPath}/out` のように先頭がプレースホルダーのものは、実行時まで分からないので検査しません |
| `shell` | この OS で使える名前であること（Windows: `cmd` / `powershell` / `pwsh`、Linux: `bash` / `sh` / `pwsh`）。さらに、その[実行ファイルが見つかる](#実行ファイルの存在チェック)こと |
| `program` | [実行ファイルが見つかる](#実行ファイルの存在チェック)こと |
| `working_dir` | 空文字 `""` か、**存在するフォルダ**であること |
| プレースホルダー | `destination` / `command` / `args` では、[使える名前](#プレースホルダー)だけであること<br>`program` / `working_dir` では**使えない**こと（実行時に置き換えないため） |

#### 検査しないこと

起動時には分からない、または実行するたびに変わるものは検査しません。実行時に失敗した場合はアクションの失敗としてログに残ります。

- プレースホルダーを展開した**後**のパスが存在するか
- `destination` に書き込めるか（権限・空き容量）
- `command` の中身が正しいコマンドか
- 監視するフォルダに読み取り権限があるか
...
```
  [1] rules "backup" > actions[1] > shell
      シェル 'pwsh' の実行ファイル 'pwsh.exe' が PATH 上に見つかりません
      探した PATH（3 件）:
        C:\WINDOWS\system32
        C:\WINDOWS
...
見つからない理由によって、出る内容が変わります。

| 状況 | 表示 |
|---|---|
| 名前だけの指定（`pwsh` など）で、PATH のどこにも無い | `PATH 上に見つかりません` と、探した PATH |
| パスで指定したが、そこにファイルが無い | `存在しません` |
| ファイルはあるが実行権限が無い（Linux などの Unix 系のみ） | `実行権限がありません`。ファイルに実行権限を付ければ直ります |

> **Windows のサービスで動かす場合は特に注意してください。** 実行ファイルを探すのに使われるのは、
> **サービスの実行アカウントの PATH** です。ログオンしているユーザーの PATH ではありません。
> `run_as_logged_in_user = true` にしていても、子プロセスへ渡す環境変数の PATH と、
> 実行ファイルを探すときの PATH は別物で、後者は呼び出し元（サービス）のものが使われます。
>
> そのため「CLI では動くのに、サービスでは実行ファイルが見つからない」という状態が起きます。
> ユーザーごとのフォルダにインストールしたツールを使う場合は、`execute` でフルパス指定してください。

```toml
# PATH に頼らずフルパスで指定する（サービスでも確実に動く）
[[rules.actions]]
type        = "execute"
program     = 'C:\Tools\pwsh\pwsh.exe'
args        = ["-NoProfile", "-File", 'C:\scripts\process.ps1', "{FullName}"]
working_dir = ""
```

- 区切り文字を含む指定（絶対パス・相対パス）は、**その場所だけ**を見ます（PATH は探しません）
- 名前だけの指定は PATH を順に探します。Windows では `PATHEXT` の拡張子も試すため、`pwsh` のように拡張子を省略して書けます
- PATH 上に実行権限の無い同名ファイルと、実行できる同名ファイルが両方あるときは、OS と同じく実行できる方を使います
