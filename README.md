...
| プレースホルダー | 置き換わる値 | 例 |
|---|---|---|
| `{Date}` | ログを書き出した日の日付 | `system_{Date}.log` → `system_20240302.log` |
| `{DateTime}` | ログを書き出した日時 | `system_{DateTime}.log` → `system_20240302_103020.log` |

- ファイル名は**ログを書き出すたびに**その時点の日時で決まります
  - `{Date}` なら、日付が変わると自動で新しいファイルに書き始めます（1 日 1 ファイル）
  - `{DateTime}` は秒まで入るので、書き出した時刻が違えば**別のファイル**になります。1 つのファイルにまとめたい場合は `{Date}` を使ってください
